...
帧的root位置 转化为 与前一帧的位置差 ，然后投影到当前帧的旋转矩阵的位置
%}

function [channels_in_local_coordinates,skel,R0,T0] = changeCoordinateSpace(angleSkel,poseData)
    %/addpaths;
    [skel,channels] = getExpmapFromSkeleton(angleSkel,poseData);
    channels_in_local_coordinates = channels;  % 转变为exp map
    rootRotInd = skel.tree(1).expmapInd;
    data = channels(1,:);  % 第一行
    R_prev = expmap2rotmat(data(rootRotInd)); % 旋转矩阵
    T_prev = data(1:3);  % 平移矩阵
    R0 = R_prev;
    T0 = T_prev;

    for ii=1:size(channels,1)
        data = channels(ii,:);
        R = expmap2rotmat(data(rootRotInd));%每一帧root 的旋转矩阵
        T = data(1:3);%每一帧root的平移矩阵
        % https://math.stackexchange.com/questions/87338/change-in-rotation-matrix
        R_diff = R*(R_prev^-1); % inverse of rotation matrix is its tranpose,
        T_diff = T - T_prev;
